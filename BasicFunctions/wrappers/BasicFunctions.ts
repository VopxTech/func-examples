...
export class BasicFunctions implements Contract {
    ...
    static createFromConfig(config: BasicFunctionsConfig, code: Cell, workchain = 0) {
    ...
    async sendIfStatement(
        ...
        await provider.internal(via, {
            value: opts.value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(1, 32).storeUint(opts.increaseBy, 32).endCell(),
        });
    }
    ...
    async sendRepeatLoop(
        ...
        await provider.internal(via, {
            value: opts.value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(2, 32).storeUint(opts.increaseBy, 32).endCell(),
        });
    }
    ...
    async sendWhileLoop(
        ...
        await provider.internal(via, {
            value: opts.value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(3, 32).storeUint(opts.limit, 32).endCell(),
        });
    }
    ...
    async sendUntilLoop(
        ...
        await provider.internal(via, {
            value: opts.value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(4, 32).storeUint(opts.limit, 32).endCell(),
        });
    }
